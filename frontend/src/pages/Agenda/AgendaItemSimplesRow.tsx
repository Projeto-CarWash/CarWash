...
export function AgendaItemSimplesRow({
  ...
  const content = (
    <>
      <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground sm:w-64 sm:shrink-0 dark:text-zinc-200">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-xs text-muted-foreground dark:text-zinc-400">
          {item.servicosResumo}
        </p>
      </div>

      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground sm:w-56 sm:shrink-0 dark:text-zinc-400">
  ...
  return (
    <li
      className="flex flex-col gap-2 rounded-xl border border-border bg-white/60 p-4 sm:flex-row sm:items-center sm:gap-4 dark:border-zinc-800/60 dark:bg-zinc-900/30"
      aria-label={descricao}
    >
      {content}
    </li>
