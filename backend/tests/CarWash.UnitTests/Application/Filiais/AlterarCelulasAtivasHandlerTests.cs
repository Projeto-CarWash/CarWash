...
public class AlterarCelulasAtivasHandlerTests
{
    ...
    public async Task Valor_diferente_ajusta_persiste_audita_e_retorna_response()
    {
        ...
        _repo.ObterPorIdRastreadoAsync(filial.Id, Arg.Any<CancellationToken>()).Returns(filial);
    ...
    {
        _repo.ObterPorIdRastreadoAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>()).Returns((Filial?)null);
    ...
    private static Filial NovaFilial(int celulas) =>
        Filial.Criar(Guid.NewGuid(), "Filial Centro", celulas, "America/Sao_Paulo");
}
