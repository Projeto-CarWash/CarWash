...
/// <summary>
/// Cobre o RF010 — <c>PATCH /api/v1/agendamentos/{id}</c> ponta a
/// ponta com Testcontainers + PostgreSQL real. Valida o checklist QA do card 137
/// (200, 400, 401, 404, 409).
/// </summary>
[Collection(nameof(PostgresCollection))]
public class EditarAgendamentoEndpointTests : IAsyncDisposable
{
    ...
    public EditarAgendamentoEndpointTests(PostgresFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task PATCH_editar_agendado_retorna_200()
    {
        ...
        var response = await client.PatchAsJsonAsync(
            RotaEditar(agendamentoId),
            new { observacoes = "Observação editada" },
            _json);

        response.StatusCode.Should().Be(HttpStatusCode.OK);

        var corpo = await response.Content.ReadFromJsonAsync<JsonElement>(_json);
        corpo.GetProperty("message").GetString().Should().Be("Agendamento atualizado com sucesso.");
        ...
        data.GetProperty("atualizadoEm").GetDateTime().Should().BeCloseTo(DateTime.UtcNow, TimeSpan.FromSeconds(10));
    }
    ...
    [Fact]
    public async Task PATCH_editar_finalizado_retorna_409_com_mensagem_finalizado()
    {
        var client = await AuthenticatedHttpClient.CreateAsync(_factory);
        var agendamentoId = await CriarAgendamentoAsync(client);
        ...
        var response = await client.PatchAsJsonAsync(
            RotaEditar(agendamentoId),
            new { observacoes = "Tentativa de editar finalizado" },
            _json);

        response.StatusCode.Should().Be(HttpStatusCode.Conflict);
        var corpo = await response.Content.ReadFromJsonAsync<JsonElement>(_json);
        corpo.GetProperty("type").GetString().Should().Contain("agendamento-edicao-bloqueada");
        corpo.GetProperty("title").GetString().Should().Contain("finalizado");
    }

    [Fact]
    public async Task PATCH_editar_cancelado_retorna_409()
    {
        var client = await AuthenticatedHttpClient.CreateAsync(_factory);
        var agendamentoId = await CriarAgendamentoAsync(client);
        await TransicionarViaDbAsync(agendamentoId, StatusAgendamento.Cancelado);

        var response = await client.PatchAsJsonAsync(
            RotaEditar(agendamentoId),
            new { observacoes = "Tentativa de editar cancelado" },
            _json);

        response.StatusCode.Should().Be(HttpStatusCode.Conflict);
        var corpo = await response.Content.ReadFromJsonAsync<JsonElement>(_json);
        corpo.GetProperty("type").GetString().Should().Contain("agendamento-edicao-bloqueada");
        corpo.GetProperty("title").GetString().Should().Contain("cancelado");
    }

    [Fact]
    public async Task PATCH_editar_em_andamento_retorna_409()
    {
        var client = await AuthenticatedHttpClient.CreateAsync(_factory);
        var agendamentoId = await CriarAgendamentoAsync(client);
        ...
        var response = await client.PatchAsJsonAsync(
            RotaEditar(agendamentoId),
            new { observacoes = "Tentativa de editar em andamento" },
            _json);

        response.StatusCode.Should().Be(HttpStatusCode.Conflict);
        var corpo = await response.Content.ReadFromJsonAsync<JsonElement>(_json);
        corpo.GetProperty("type").GetString().Should().Contain("agendamento-edicao-bloqueada");
        corpo.GetProperty("title").GetString().Should().Contain("não permite edição");
    }
    ...
    private async Task TransicionarViaDbAsync(Guid agendamentoId, StatusAgendamento novoStatus)
    {
        ...
        {
            agendamento!.Cancelar("Cancelamento para teste de edição", Guid.NewGuid());
        }
